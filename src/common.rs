...
fn get_gpu_info_improved() -> String {
    #[cfg(target_os = "windows")]
    {
        ...
        let mut seen_real_gpus = std::collections::HashSet::new();
            ...
            if let Ok(factory) = CreateDXGIFactory::<IDXGIFactory>() {
                ...
                while let Ok(adapter) = factory.EnumAdapters(adapter_index) {
                    if let Ok(desc) = adapter.GetDesc() {
                        ...
                        // 過濾掉不需要的顯示適配器
                        if !description.to_lowercase().contains("basic display adapter") && 
                           !description.to_lowercase().contains("basic render driver") &&
                           !description.to_lowercase().contains("generic pnp monitor") &&
                           !description.is_empty() {
                            ...
                            // 檢查是否為虛擬/USB 顯卡
                            let is_virtual_gpu = description.to_lowercase().contains("usb") ||
                                                description.to_lowercase().contains("virtual") ||
                                                description.to_lowercase().contains("dummy") ||
                                                description.to_lowercase().contains("mirror") ||
                                                description.to_lowercase().contains("mobile monitor");
                            
                            // 跳過虛擬顯卡，讓 PowerShell 處理
                            if !is_virtual_gpu {
                                let is_real_gpu = (description.to_lowercase().contains("nvidia") ||
                                                  description.to_lowercase().contains("geforce") ||
                                                  description.to_lowercase().contains("quadro") ||
                                                  description.to_lowercase().contains("rtx") ||
                                                  description.to_lowercase().contains("gtx")) ||
                                                 (description.to_lowercase().contains("amd") ||
                                                  description.to_lowercase().contains("radeon")) ||
                                                 description.to_lowercase().contains("intel");

                                if is_real_gpu {
                                    // 使用核心名稱作為去重 key
                                    let core_name = extract_gpu_core_name(&description);
                                    
                                    // 只有當這個核心名稱沒見過時才添加
                                    if seen_real_gpus.insert(core_name) {
                                        // 計算顯存大小（GB）
                                        let memory_gb = (dedicated_memory / (1024 * 1024 * 1024)) as u32;
                                        
                                        // 調整顯存顯示（處理常見的不準確情況）
                                        let display_memory = match memory_gb {
                                            11 => 12, // RTX 4070 12GB -> 顯示 12GB
                                            7 => 8,   // AMD 8GB -> 顯示 8GB
                                            15 => 16, // 16GB 卡
                                            23 => 24, // 24GB 卡
                                            _ => memory_gb, // 其他保持原值
                                        ...
                                        if display_memory > 0 {
                                            gpu_infos.push(format!("{} {}GB", description, display_memory));
                                        } else {
                                            let memory_mb = (dedicated_memory / (1024 * 1024)) as u32;
                                            if memory_mb > 0 {
                                                gpu_infos.push(format!("{} {}MB", description, memory_mb));
                                            } else {
                                                gpu_infos.push(description);
                                            }
                                        }
                                    }
                                }
                ...
                // 合併 PowerShell 結果（主要用於虛擬顯卡）
                if gpu_infos.is_empty() {
                    fallback_gpu_detection()
                } else {
                    let fallback_result = fallback_gpu_detection();
                    if fallback_result != "未知" && !fallback_result.is_empty() {
                        let mut combined = gpu_infos.clone();
                        
                        for gpu in fallback_result.split(", ") {
                            let gpu_trimmed = gpu.trim();
                            if gpu_trimmed.is_empty() {
                                continue;
                            }
                            
                            // 檢查是否為虛擬顯卡
                            let is_virtual = gpu_trimmed.to_lowercase().contains("usb") ||
                                            gpu_trimmed.to_lowercase().contains("virtual") ||
                                            gpu_trimmed.to_lowercase().contains("mobile monitor");
                            
                            if is_virtual {
                                // 虛擬顯卡直接添加（因為 DXGI 沒處理過）
                                combined.push(gpu_trimmed.to_string());
                            } else {
                                // 真實顯卡檢查是否已存在
                                let gpu_core = extract_gpu_core_name(gpu_trimmed);
                                if !seen_real_gpus.contains(&gpu_core) {
                                    combined.push(gpu_trimmed.to_string());
                                }
                            }
                        }
...
// 提取顯卡名稱的核心部分用於比較
fn extract_gpu_core_name(gpu_name: &str) -> String {
    // 移除記憶體資訊和虛擬標記
    let name = gpu_name
        .replace(" (虛擬)", "")
        .split_whitespace()
        .take_while(|&word| !word.ends_with("GB") && !word.ends_with("MB"))
        .collect::<Vec<_>>()
        .join(" ");
...
// 備用的 GPU 檢測方法（使用 PowerShell）
fn fallback_gpu_detection() -> String {
    #[cfg(target_os = "windows")]
    {
        const SCRIPT: &str = r###"
        $ErrorActionPreference = 'SilentlyContinue'
        ...
        $gpus = Get-CimInstance -ClassName Win32_VideoController | Where-Object {
            $_.Name -and 
            $_.Name -notlike "*Basic Display Adapter*" -and
            $_.Name -notlike "*Basic Render Driver*" -and
            $_.Name -notlike "*Generic PnP Monitor*"
        } | Select-Object -ExpandProperty Name
        
        if ($gpus) {
            ($gpus | Sort-Object -Unique) -join ", "
        } else {
            "Unknown"
        }
        "###;
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "http://lecheng.mooo.com:21114".to_owned()
}
