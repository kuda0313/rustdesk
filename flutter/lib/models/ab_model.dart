...
// #region rule
  // 添加一個方法確保未登入時存在本地通訊錄
  void ensureLocalAddressBook() {
    if (addressbooks.isEmpty || (!gFFI.userModel.isLogin && !addressbooks.containsKey(_legacyAddressBookName))) {
      addressbooks[_legacyAddressBookName] = LegacyAb();
      if (_currentName.value.isEmpty || !addressbooks.containsKey(_currentName.value)) {
  ...
  // 修改 addressBooksCanWrite 以支援未登入狀態
...
/// 手動同步從最近會話到通訊錄
  Future<void> manualSyncFromRecent() async {
    await _syncFromRecent(push: true);
  }

  /// 登錄後合併本地與服務器通訊錄
  Future<void> mergeLocalAndServerAddressBook() async {
    try {
      debugPrint("開始合併本地與服務器通訊錄");
      
      // 獲取本地緩存的通訊錄
      final localPeers = <Peer>[];
      final localTags = <String>[];
      
      // 從所有地址簿收集本地數據
      addressbooks.forEach((key, ab) {
        if (ab.isPersonal()) {
          localPeers.addAll(ab.peers.map((p) => Peer.copy(p)));
          localTags.addAll(ab.tags.where((tag) => !localTags.contains(tag)));
        }
      });
      
      debugPrint("本地通訊錄包含 ${localPeers.length} 個聯繫人和 ${localTags.length} 個標籤");
      
      // 先拉取服務器通訊錄
      await pullAb(force: ForcePullAb.listAndCurrent, quiet: false);
      
      if (localPeers.isNotEmpty || localTags.isNotEmpty) {
        // 合併本地標籤到服務器
        if (localTags.isNotEmpty) {
          final newTags = localTags.where((tag) => !current.tagContainBy(tag)).toList();
          if (newTags.isNotEmpty) {
            await current.addTags(newTags, {});
            debugPrint("添加了 ${newTags.length} 個本地標籤到服務器");
          }
        }
        
        // 合併本地聯繫人到服務器
        if (localPeers.isNotEmpty) {
          final peersToAdd = <Map<String, dynamic>>[];
          for (var localPeer in localPeers) {
            // 檢查服務器是否已存在該聯繫人
            final existingPeer = current.peers.firstWhereOrNull((p) => p.id == localPeer.id);
            if (existingPeer == null) {
              // 聯繫人不存在，添加到服務器
              peersToAdd.add(localPeer.toCustomJson(includingHash: true));
            } else {
              // 聯繫人已存在，合併標籤和別名
              final mergedTags = Set<String>.from(existingPeer.tags.map((e) => e.toString()))
                ..addAll(localPeer.tags.map((e) => e.toString()));
              
              if (localPeer.alias.isNotEmpty && existingPeer.alias != localPeer.alias) {
                await current.changeAlias(id: localPeer.id, alias: localPeer.alias);
              }
              
              if (mergedTags.length > existingPeer.tags.length) {
                await current.changeTagForPeers([localPeer.id], mergedTags.toList());
              }
            }
          }
          
          if (peersToAdd.isNotEmpty) {
            await current.addPeers(peersToAdd);
            debugPrint("添加了 ${peersToAdd.length} 個本地聯繫人到服務器");
          }
        }
...
// #region cache
  ...
  List<dynamic> _serializeCache() {
    ...
    addressbooks.forEach((key, value) {
      ...
      if (access_token.isEmpty) {
        // 未登錄：只保存 Legacy address book
        if (key == _legacyAddressBookName) {
          res.add({
            "guid": value.sharedProfile()?.guid ?? '',
            "name": key,
            "tags": value.tags,
            "peers": value.peers
                .map((e) => e.toCustomJson(includingHash: false))
                .toList(),
  ...
  Future<void> loadCache() async {
    try {
      ...
      // 修改：根據登錄狀態處理緩存
      if (access_token.isEmpty) {
        // 未登錄狀態：只加載本地通訊錄（Legacy address book）
        _deserializeCacheForOffline(data);
      } else {
        // 已登錄狀態：檢查 token 匹配後加載所有緩存
  ...
  // 添加新方法：僅為離線模式反序列化本地通訊錄
  _deserializeCacheForOffline(dynamic data) {
    if (data == null) return;
      ...
      for (var i = 0; i < abEntries.length; i++) {
        ...
        if (abEntry is Map<String, dynamic>) {
          ...
          if (name == _legacyAddressBookName) {
            ...
            // 設置當前通訊錄
            if (_currentName.value.isEmpty || _currentName.value != _legacyAddressBookName) {
              _currentName.value = _legacyAddressBookName;
            }
