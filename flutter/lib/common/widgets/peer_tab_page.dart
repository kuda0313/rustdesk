...
class _PeerTabPageState extends State<PeerTabPage>
    ...
    _TabEntry(
        ...
        ({dynamic hint}) async {
          if (gFFI.userModel.isLogin) {
            // 如果已登錄，執行手動同步
            await gFFI.abModel.manualSyncFromRecent();
            await gFFI.abModel.pullAb(
                force: hint == null ? ForcePullAb.current : null,
                quiet: false);
          } else {
            // 如果未登錄，只加載本地緩存
  ...
  Widget _createRefresh(
    ...
    return Offstage(
      ...
      child: Tooltip(
        // 修改：針對通訊錄頁面顯示不同的提示文本
        message: index == PeerTabIndex.ab 
            ? (gFFI.userModel.isLogin ? translate('Manual Sync') : translate('Refresh'))
            : translate('Refresh'),
        child: RefreshWidget(
            onPressed: () {
