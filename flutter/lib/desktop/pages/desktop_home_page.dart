...
String? _cachedSysinfo;
  ...
  // 新增：右邊視窗模式控制
  final RxBool _isHardwareMode = false.obs;
  bool _isLoadingHardware = false;
...
final List<Map<String, String>> _hardwareFields = [
  ...
  // 硬體相關
  // 新增：模式切換按鍵
  Widget _buildModeToggleButtons(BuildContext context) {
        ...
        children: [
          Expanded(
            child: _buildToggleButton(
              context,
              "連線模式",
              !_isHardwareMode.value,
              () => _switchToConnectionMode(),
            ),
          ),
          SizedBox(width: 8),
          Expanded(
  ...
  // 切換到連線模式
  void _switchToConnectionMode() {
    _isHardwareMode.value = false;
  }

  // 切換到硬體檢測模式
  void _switchToHardwareMode() {
    _isHardwareMode.value = true;
    // 只有在切換到硬體模式時才開始檢測
    if (_parsedSysinfo == null && !_isLoadingHardware) {
      _loadHardwareInfo();
    }
  }

  // 載入硬體資訊
  void _loadHardwareInfo() async {
    if (_isLoadingHardware) return;
    
    setState(() {
      _isLoadingHardware = true;
    });
  ...
  buildRightPane(BuildContext context) {
    return Obx(() => Container(
      ...
      child: _isHardwareMode.value 
          ? _buildHardwarePane(context)
          : ConnectionPage(),
    ));
  }

  // 硬體檢測面板
  Widget _buildHardwarePane(BuildContext context) {
    return Column(
      children: [
        Expanded(
          child: Column(
  ...
  void _copySystemInfo() {
    try {
      ...
      if (memoryDetails != null && 
        ...
        for (var line in detailLines) {
          if (line.trim().isNotEmpty) {
            infoLines.add('          ${line.trim()}'); // 10個空格縮排
          }
        }
      }
      ...
      if (diskInfo != null && diskInfo.toString().isNotEmpty) {
        ...
        if (diskLines.isNotEmpty) {
          for (int i = 0; i < diskLines.length; i++) {
            ...
            // 尋找容量信息的開始位置（通常是盤符:，如 C:, D:, E: 等）
            RegExp drivePattern = RegExp(r'\s+[A-Z]:\d');
            Match? match = drivePattern.firstMatch(line);
            
            if (match != null) {
              final splitIndex = match.start;
              final model = line.substring(0, splitIndex).trim();
              final capacity = line.substring(splitIndex).trim();
              
              String displayText;
              if (i == 0) {
                // 第一行：硬碟型號: + 型號 + 兩個TAB + 容量
                displayText = '硬碟型號: ${model}\t\t${capacity}';
              } else {
                // 第2行之後：10格空格 + 型號 + 兩個TAB + 容量
                displayText = '          ${model}\t\t${capacity}';
              }
              infoLines.add(displayText);
            } else {
              // 如果沒有找到標準格式
              if (i == 0) {
                infoLines.add('硬碟型號: ${line}');
              } else {
                infoLines.add('          ${line}'); // 10個空格縮排
              }
            }
          }
        } else {
      ...
      // 處理顯卡資訊（支持多行）
      final gpuInfo = sysinfo['gpu'];
      if (gpuInfo != null && gpuInfo.toString().isNotEmpty) {
        final gpuLines = gpuInfo.toString().split('\n').where((line) => line.trim().isNotEmpty).toList();
        
        if (gpuLines.isNotEmpty) {
          for (int i = 0; i < gpuLines.length; i++) {
            final line = gpuLines[i].trim();
            if (i == 0) {
              infoLines.add('顯 示 卡: ${line}');
            } else {
              infoLines.add('          ${line}'); // 10個空格縮排
            }
          }
        } else {
          infoLines.add('顯 示 卡: 未知');
        }
      } else {
        infoLines.add('顯 示 卡: 未知');
      }
  ...
  Widget _buildSystemInfo(Map<String, dynamic> sysinfo) {
    return Column(
      children: [
        ...
        _buildGpuInfo(sysinfo['gpu'] ?? '未知'), // 使用新的顯卡顯示函數
  ...
  // 新增顯卡多行顯示函數
  Widget _buildGpuInfo(String gpuInfo) {
    final gpuLines = gpuInfo.split('\n').where((line) => line.trim().isNotEmpty).toList();
    
    if (gpuLines.isEmpty) {
      return _buildInfoRow("顯  示  卡", "未知");
    }
    
    return Column(
      children: gpuLines.asMap().entries.map((entry) {
        final index = entry.key;
        final line = entry.value.trim();
